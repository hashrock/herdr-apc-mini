...
実機が無くてもデーモンは起動し、挿されるまで待つ。抜かれたら送信をやめ、挿し直せば
繋ぎ直して盤面を描き直す。

CoreMIDI のポート一覧は**スレッドごとにキャッシュ**されていて、run loop を回した
スレッドしか更新されない。クライアントを作り直しても駄目で、素直に sleep で待つと
挿し直しに永久に気づけない。そこで見張りスレッドは `CFRunLoopRunInMode` で待ち、
繋ぎにいく main 側は直前に同じものを 0 秒で回して一覧を更新する。

ポートの有無を見るだけでは、**見張りの間隔より速い抜き差しを取りこぼす**。抜けたことに
気づかないまま繋がっている扱いが続き、実機は消灯しているのに差分しか送られない
（＝一部のパッドしか光らない）。そこで `SetupChanged` の通知も一緒に見て、通知が
来たら有無にかかわらず繋ぎ直して丸ごと描き直す。通知は MIDI 機器なら何の抜き差しでも
飛んでくるが、そのたびに一度描き直すだけなので実害はない。

実機を抜かずに試すには `cargo run --bin fakeport -- "APC mini mk2 Control" 8` で
仮想ポートを一定時間だけ出す。
