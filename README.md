...
| 操作 | 動作 |
|---|---|
| 短押し（0.8 秒未満） | `workspace.focus` — そのワークスペースへ飛ぶ |
| 長押し（0.8 秒以上） | `pane.zoom` — そのペインを全画面トグル |

どちらも発火時に白フラッシュする。長押しは**指を離す前に**発火するので、
押しっぱなしのまま結果が分かる。

列の割り当ては `~/.config/herdr/plugins/config/apc-mini/pad-map.json` に永続化される。
初回だけエージェントのいるワークスペースを前に詰め、以後は順序が動かない。
